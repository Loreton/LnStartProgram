...
class nullLogger():
    ...
    def dummy(self,  title, extra_data=None): pass
    info    = dummy
    debug1  = dummy
    debug2  = dummy
    debug3  = dummy
    ...
    def console(self,  title, extra_data=None):
...
class myLogger():
    def __init__(self, filename, debug_level=0, dry_run=False, log_modules=[], color=None):
        global C
    ...
    def _printLog(self, level=None, title=None, extra_data=None):
...
def prepareData(title, extra_data):
    ...
    if extra_data in ['', None]:  # devo evitare  None e False
        ...
        # elif 'dotmap' in str(type(extra_data)).lower():
        #     print ('dotamp')
        #     _myDict = extra_data.toDict()
        #     data_list.append(title + '[DotMap]:')
        #     my_json = json.dumps(_myDict, indent=4, sort_keys=True)
